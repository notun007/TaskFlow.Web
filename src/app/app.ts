...
import { ApiTask, ApiTaskDetails, AuditListItem, ProjectDetails, ProjectListItem, ReferenceItem, SoftwareListItem, TaskApiService, TeamListItem, VendorListItem } from './task-api.service';
...
export class App implements OnInit {
  ...
  setNav(item: string) {
    ...
    if (['Software', 'Teams', 'Vendors', 'Audit Log'].includes(item)) this.loadOperationalWorkspace(item);
  }
  ...
  private loadOperationalWorkspace(section: string) {
    ...
    const request = (section === 'Software' ? this.taskApi.getSoftwareList()
      ...
      : this.taskApi.getAuditLog()) as Observable<SoftwareListItem[] | TeamListItem[] | VendorListItem[] | AuditListItem[]>;
    request.subscribe({
      next: records => {
